...
} crypto_hash_sha512_state_;

int
crypto_hash_sha512_init_(crypto_hash_sha512_state_ *state) {
...
void
sodium_memzero_(void *const pnt, const size_t len) {
#ifdef _WIN32
...
static void
SHA512_Transform_(uint64_t *state, const unsigned char block[128]) {
    uint64_t W[80];
    uint64_t S[8];
    uint64_t t0, t1;
    int i;
    ...
    sodium_memzero_((void *) W, sizeof W);
    sodium_memzero_((void *) S, sizeof S);
    sodium_memzero_((void *) &t0, sizeof t0);
    sodium_memzero_((void *) &t1, sizeof t1);
}

int
crypto_hash_sha512_update_(crypto_hash_sha512_state_ *state,
    ...
    SHA512_Transform_(state->state, state->buf);
    src += 128 - r;
    inlen -= 128 - r;

    while (inlen >= 128) {
        SHA512_Transform_(state->state, src);
        src += 128;
        inlen -= 128;
    }
...
void
SHA512_Pad_(crypto_hash_sha512_state_ *state) {
    ...
    crypto_hash_sha512_update_(state, PAD, (unsigned long long) plen);

    crypto_hash_sha512_update_(state, len, 16);
}

int
crypto_hash_sha512_final_(crypto_hash_sha512_state_ *state,
                          unsigned char *out) {
    SHA512_Pad_(state);
    be64enc_vect(out, state->state, 64);
    sodium_memzero_((void *) state, sizeof *state);

    return 0;
}

int AES_PairVerify1(unsigned char *a1, unsigned char *a2, unsigned int a2_len, unsigned char *a3,
                    unsigned int a3_len, unsigned char *a4, unsigned char *a5) {
    crypto_hash_sha512_state_ hs;

    //unsigned char az[64];
    //unsigned char nonce[64];
    //unsigned char hram[64];

    crypto_hash_sha512_init_(&hs);
    crypto_hash_sha512_update_(&hs, a2, a2_len);
    crypto_hash_sha512_update_(&hs, a1 /*+ 220*/, 32);
    crypto_hash_sha512_final_(&hs, a4);

    crypto_hash_sha512_init_(&hs);
    crypto_hash_sha512_update_(&hs, a3, a3_len);
    crypto_hash_sha512_update_(&hs, a1 /*+ 220*/, 32);
    crypto_hash_sha512_final_(&hs, a5);
 ...
 */
int AES_SetEncryptKey(const unsigned char *userKey, const int bits, /*AES_KEY*/aes_key_t *key) {
    u32 *rk;
    int i = 0;
    u32 temp;
...
void AES_PairVerify2(unsigned char *a1, unsigned char *aes_key, unsigned char *aes_iv) {
    int v3; // eax@1
    int v4; // esi@1

    unsigned char *temp = (unsigned char *) malloc(284);

    memset(temp, 0, 284);

    *(unsigned int *) (temp + 280) = 0;
    *(unsigned int *) (temp + 276) = 0;

    memcpy(temp + 244, aes_iv, 16);
    memset(temp + 260, 0, 16);

    //AES_KEY?„ç??„å?244byte

    AES_SetEncryptKey(aes_key, 128, (/*AES_KEY*/aes_key_t *) temp);

    //sub_10341F80(aes_key, 128, temp);

    *(unsigned int *) a1 = (unsigned int) temp;
}
 ...
 */
void AES_Encrypt(const unsigned char *in, unsigned char *out, const /*AES_KEY*/aes_key_t *key) {
    const u32 *rk;
...
void AES_PairVerify3(const unsigned char *in, unsigned char *out,
                     const unsigned long length, const /*AES_KEY*/aes_key_t *key,
...
#if 0 //FIXME: Incomplete porting API -> AES_decrypt().
    else if (in != out)
    {
        while (len >= AES_BLOCK_SIZE)
        {
...
int AES_FpSetup(unsigned char *a1, unsigned char *a2, unsigned int a3, unsigned char *a4,
    ...
    crypto_hash_sha512_init_(&hs);
    crypto_hash_sha512_update_(&hs, a2, a3);
    crypto_hash_sha512_update_(&hs, a1 /*+ 220*/, 32);
    crypto_hash_sha512_final_(&hs, a6);

    return 0;
}

static unsigned char airplay_streamkey_cnt = 0;
static unsigned char airplay_streamkey[32 * 5];

unsigned char *AES_AirplayStreamKeyInit(char a1, char a2, char a3, char a4, char a5, char a6,
                                        char a7, char a8, char a9, char a10, char a11, char a12,
                                        char a13, char a14, char a15, char a16, char a17, char a18,
                                        char a19, char a20, char a21, char a22, char a23, char a24,
                                        char a25, char a26, char a27, char a28, char a29, char a30,
                                        char a31) {
    int v31; // esi@1
    int v32; // edx@3
    int v33; // esi@3

    v31 = *((unsigned int *) airplay_streamkey);

    if (airplay_streamkey_cnt >= 5) {
        airplay_streamkey_cnt = 0;
    }

    v32 = 32 * airplay_streamkey_cnt;

    airplay_streamkey[v32 + 0] = a1 ^ 0x5D;
    airplay_streamkey[v32 + 1] = a2 ^ 0x8E;
    airplay_streamkey[v32 + 2] = a3 ^ 0x50;
    airplay_streamkey[v32 + 3] = a4 ^ 0x9C;
    airplay_streamkey[v32 + 4] = a5 ^ 0x1E;
    airplay_streamkey[v32 + 5] = a6 ^ 0xE0;
    airplay_streamkey[v32 + 6] = a7 ^ 0x6F;
    airplay_streamkey[v32 + 7] = a8 ^ 0x63;
    airplay_streamkey[v32 + 8] = a9 ^ 0xFB;
    airplay_streamkey[v32 + 9] = a10 ^ 0xCF;
    airplay_streamkey[v32 + 10] = a11 ^ 0x14;
    airplay_streamkey[v32 + 11] = a12 ^ 0x5D;
    airplay_streamkey[v32 + 12] = a13 ^ 0x9B;
    airplay_streamkey[v32 + 13] = a14 ^ 0xB9;
    airplay_streamkey[v32 + 14] = a15 ^ 0x36;
    airplay_streamkey[v32 + 15] = a16 ^ 0x6C;
    airplay_streamkey[v32 + 16] = a17 ^ 0xE2;
    airplay_streamkey[v32 + 17] = a18 ^ 1;
    airplay_streamkey[v32 + 18] = a19 ^ 0x26;
    airplay_streamkey[v32 + 19] = a20 ^ 0xE7;
    airplay_streamkey[v32 + 20] = a21 ^ 0x73;
    airplay_streamkey[v32 + 21] = a22 ^ 0xD4;
    airplay_streamkey[v32 + 22] = a23 ^ 0x6E;
    airplay_streamkey[v32 + 23] = a24 ^ 0x44;
    airplay_streamkey[v32 + 24] = a25 ^ 0x95;
    airplay_streamkey[v32 + 25] = a26 ^ 0x99;
    airplay_streamkey[v32 + 26] = a27 ^ 0xBE;
    airplay_streamkey[v32 + 27] = a28 ^ 0x39;
    airplay_streamkey[v32 + 28] = a29 ^ 0xC5;
    airplay_streamkey[v32 + 29] = a30 ^ 0x2F;
    airplay_streamkey[v32 + 30] = a31 ^ 0xAE;
    airplay_streamkey[v32 + 31] = a31 ^ 0xFB;

    airplay_streamkey_cnt = airplay_streamkey_cnt + 1;

    return (airplay_streamkey + v32);
}

int AES_AirplayStreamKeyDecrypt(/*unsigned char *a1,*/ unsigned char *a2, unsigned char *a3,
    ...
    crypto_hash_sha512_init_(&hs);
    crypto_hash_sha512_update_(&hs, a2, strlen((const char *) a2));
    crypto_hash_sha512_update_(&hs, a4, 16);
    crypto_hash_sha512_final_(&hs, a5);

    crypto_hash_sha512_init_(&hs);
    crypto_hash_sha512_update_(&hs, a3, strlen((const char *) a3));
    crypto_hash_sha512_update_(&hs, a4, 16);
    crypto_hash_sha512_final_(&hs, a6);
